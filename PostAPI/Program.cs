...
builder.Services.AddSwaggerGen(c =>
{
    c.SwaggerDoc("v1", new OpenApiInfo
    {
        Title = "PostAPI",
        Version = "v1",
        Description = "API for managing posts in the SocialBook application."
    });
...
if (app.Environment.IsDevelopment())
{
    ...
    app.UseSwaggerUI(c =>
    {
        c.SwaggerEndpoint("/swagger/v1/swagger.json", "PostAPI V1");
...
using (var scope = app.Services.CreateScope())
{
    ...
    try
    {
        context.Database.Migrate();  // ✅ Applies migrations to `PostDB`
    }
    catch (Exception ex)
    {
        Console.WriteLine($"PostAPI migration failed: {ex.Message}");
    }
}
